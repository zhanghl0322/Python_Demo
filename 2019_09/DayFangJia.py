    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            sql = "insert into lianjia_house (`id`, `positionInfo`,`houseInfo`,`priceInfo`," \
                  "`followInfo`,`tag`,`titleinfo`,`addtime`,`area`) " \
                  "values (%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            # 执行sql语句
            # ['配套成熟 交通便利 户型方正 住宅', '麓湖路3号大院    -  小北 ', '3室1厅 | 87.17平米 | 西南 | 简装 | 中楼层(共5层) | 1990年建 | 塔楼', '5人关注 / 1个月以前发布', 'VR房源房本满五年', '345万单价39578元/平米']
            cursor.execute(sql, (
                0, house_list[1], house_list[2], house_list[5], house_list[3], house_list[5], house_list[0],
                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), 'GZ'))
            print(house_list[1])
            # 提交数据库
            connection.commit()
    finally:
...
def crawl_main():
    ...
    retry_times = 6
    while True:
        # 链家数据链接,通过更改page参数的值来循环读取多页评论信息
        i = i + 1
        url = URL + str(i)
        if getHouseInfo(url) == -1:
